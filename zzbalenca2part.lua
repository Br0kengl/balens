...
local menu do
    ...
    menu.information = {
        ['about'] = {
            warning = gui.label(string.format('✧ %s ~ %s', context.information.script, _DEBUG and 'v2' or 'v2')),
        }
    }
    ...
    menu.general = {
        ...
        ['world'] = {
            ['aspectratio'] = {
                ...
                value = gui.slider(translate('Aspect Ratio Value', 'Значение соотношения сторон'), 10, 200, 177, true, '', 1, { 
                    [10] = translate('Off', 'Выкл'), 
                    [125] = '5:4', 
                    [133] = '4:3', 
                    [150] = '3:2', 
                    [177] = '16:9', 
            ...
            `<root>
                <Panel class="mainmenu-navbar__btn-small mainmenu-navbar__btn-home MainMenuModeOnly">
                    <RadioButton id="main_menu"
                        ...
                        onmouseover="UiToolkitAPI.ShowTextTooltip('main_menu', 't.me/balenciagaSKEET');"
    ...
    function _panorama:create()
        ...
        if settings.background ~= _panorama.current_state.background then
            if settings.background then
                background.change("https://i.pinimg.com/1200x/07/7d/2f/077d2f3dbfce0bff62330ff7dd34b2bb.jpg")
            else
                background.restore()
            end
...
do
    ...
    function watermark:render()
        ...
        local info_text = string.format('%s ~ \ac8c8c8fe%s', context.information.username, _DEBUG and 'v2' or 'v2')
...
-- @ lol
do
    interfaces.gameui:create_command_msgbox(context.information.script, 't.me/balenciagaSKEET - BEST LUA', true)
end

-- @ auto-enable components
do
    -- Enable watermark automatically
    menu.general.visuals.widgets.watermark:set(true)
    
    -- Enable network metrics automatically
    menu.general.visuals.widgets.metrics.enable:set(false)
end
...
local detector_table = {
    nl = function(packet, target)
        if not packet or packet == ffi.NULL then
            return false
        end
    ...
    ot = function(packet, target)
        ...
        detection_storage_table.ot[target][#detection_storage_table.ot[target] + 1] = {
            ...
            uncompressed_sample_offset = packet.uncompressed_sample_offset
        }
    ...
    gs = function(packet, target)
        ...
        local sig = ("%.02X"):format(ffi.cast("uint16_t*", ffi.cast("uintptr_t", packet) + 22)[0])
        local sequence_bytes = type(packet.sequence_bytes) == "string" and string.sub(packet.sequence_bytes, 1, 4) or tostring(packet.sequence_bytes)
...
main_data_table.main:add_event_callback("voice", function(event)
    ...
    if not target or target < 1 or target > 64 then
        return
    end
